...
bool  Protein::LoadFromPDB(string filename)
{
	ifstream filein;
	string line;
	bool model=false;//是否是多个模型，仅处理第一个
...
void Protein::Atom2Line(const Atom& atom,string& line)
{
	char str[81];
	char name[5]="    ";
	if((int)atom.name.length()==4)//如果原子名长度为4
	{
		for(int i=0;i<(int)atom.name.length();i++)
			name[i]=atom.name[i];
	}
	else//如果长度不为4，则第一个为空格
	{
...
int Chain::ObtainContact(vector<Contact >& all_contact)
{
	...
	len=num_residue;
	//第一遍扫描，获得每个氨基酸的侧链半径和中心，侧链中心在ca向cb方向延伸至半径处，对于甘氨酸先将侧链放在Ca处
	for(i=0;i<len;i++)
	{
